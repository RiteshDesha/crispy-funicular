...
def _extract_tilr_survey_lines(
    ...
    """Aggressively detect red dashed TILR / survey-number parcel linework.

    v0.3 improves the first version in two ways:
    1. It uses a wider red/pink mask and a dilated sample mask, because scanned DP
       survey-number lines are often faded and broken into tiny dash fragments.
    2. It runs both Hough detection and connected-component/min-area-rectangle
       fallback, so even short fragmented TILR strokes can become visible CAD lines.

    These are always returned as separate LINE vectors on DP_TILR_SURVEY_LINES and
    never merged with zone patches or road widening.
    """
    if options.get("return_tilr_survey_lines", True) is False:
        return []

    profile = str(options.get("tilr_detection_profile", "aggressive_top_overlay")).lower()

    # DP sheets vary a lot: pure red, faded red, magenta-red, and dark printed red.
    red1 = cv2.inRange(hsv, np.array((0, 24, 35)), np.array((14, 255, 255)))
    red2 = cv2.inRange(hsv, np.array((162, 24, 35)), np.array((179, 255, 255)))
    pink_red = cv2.inRange(hsv, np.array((142, 18, 48)), np.array((179, 235, 255)))
    red_mask = cv2.bitwise_or(cv2.bitwise_or(red1, red2), pink_red)
    red_mask = cv2.bitwise_and(red_mask, influence)

    # Keep thin linework and small dash components, remove large compact red fills.
    red_mask = _remove_large_red_fills(red_mask)

    # Tiny scan gaps are normal. This mask is only for measuring whether a detected
    # geometric line is actually near red ink, not for returning filled blobs.
    sample_mask = cv2.dilate(red_mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7)), iterations=1)

    h, w = red_mask.shape[:2]
    base_min_len = max(14, int(min(w, h) * (0.010 if profile.startswith("aggressive") else 0.018)))
    min_len = int(options.get("tilr_min_line_px", base_min_len))
    max_gap = int(options.get("tilr_max_dash_gap_px", 58 if profile.startswith("aggressive") else 32))
    hough_threshold = int(options.get("tilr_hough_threshold", 11 if profile.startswith("aggressive") else 22))
    ...
    def add_segment(x1: int, y1: int, x2: int, y2: int, method: str, idx_hint: int) -> None:
        ...
        if length < min_len:
            return
        raw_coverage = _line_red_coverage(red_mask, (x1, y1), (x2, y2))
        near_coverage = _line_red_coverage(sample_mask, (x1, y1), (x2, y2))
        dash_score = _line_dash_score(red_mask, (x1, y1), (x2, y2))

        # For dashed lines raw coverage can be low because the line intentionally has gaps.
        # Near-coverage catches the actual intended axis of the dash sequence.
        if near_coverage < 0.11 and raw_coverage < 0.035:
            return
        if raw_coverage > 0.985 and dash_score < 0.05 and method == "hough":
            # Avoid compact solid red boundaries from patches.
            return

        conf = int(max(48, min(94, 50 + near_coverage * 28 + raw_coverage * 18 + dash_score * 16 + min(12, length / 95))))
        ...
        features.append(
            {
                ...
                "path": geometry,
                "confidence": conf,
                "reason": f"Aggressive red dashed/TILR line detection via {method}; raw_coverage={raw_coverage:.2f}, near_coverage={near_coverage:.2f}, dash_score={dash_score:.2f}",
                "color": [239, 68, 68, 255],
                "dashArray": [8, 5],
                "lengthPx": round(length, 2),
                "reviewStatus": "cloud_detected_unverified",
                "needsManualConfirmation": True,
                "source": "opencv_aggressive_red_dashed_tilr_detection",
                "traceMethod": f"cloud_cv_tilr_{method}_v3",
                "meaning": "Red dashed survey-number / TILR land-record reference line. Keep separate from zoning and road-widening layers.",
            }
        )

    # Multiple close kernels catch horizontal, vertical and diagonal/fragmented dash chains.
    close_kernels = [
        (7, 3), (13, 3), (25, 3), (41, 3),
        (3, 7), (3, 13), (3, 25), (3, 41),
        (7, 7), (13, 13),
    ...
    variant_masks: List[np.ndarray] = [red_mask]
    for kw, kh in close_kernels:
        k = cv2.getStructuringElement(cv2.MORPH_RECT, (kw, kh))
        variant_masks.append(cv2.morphologyEx(red_mask, cv2.MORPH_CLOSE, k, iterations=1))

    # Hough candidates.
    for vi, vm in enumerate(variant_masks):
        vm2 = cv2.morphologyEx(vm, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2)), iterations=1)
    ...
    # Component fallback: after directional closing, a dash sequence often becomes one long
    # thin component. Convert that component's major axis to a CAD line.
    for vi, vm in enumerate(variant_masks[1:]):
        num, labels, stats, _ = cv2.connectedComponentsWithStats(vm, connectivity=8)
        for ci in range(1, num):
            x, y, bw, bh, area = [int(v) for v in stats[ci]]
            if area < 8:
                continue
            long_side = max(bw, bh)
            ...
            if long_side < min_len:
                continue
            if short_side > max(18, long_side * 0.45):
                continue
            comp_mask = (labels[y:y+bh, x:x+bw] == ci).astype(np.uint8) * 255
            ...
            box[:, 0] += x
            box[:, 1] += y
            # Longest box diagonal gives a stable major direction endpoint pair.
            pairs = [(0, 1), (1, 2), (2, 3), (3, 0), (0, 2), (1, 3)]
            best = max(pairs, key=lambda ij: float(np.hypot(*(box[ij[1]] - box[ij[0]]))))
            p1, p2 = box[best[0]], box[best[1]]
            add_segment(int(p1[0]), int(p1[1]), int(p2[0]), int(p2[1]), "component_axis", vi * 1000 + ci)
    ...
    return features[: int(options.get("tilr_max_lines", 180))]
...
def trace(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "meta": {
            ...
            "tilrDetectionProfile": options.get("tilr_detection_profile", "aggressive_top_overlay"),
